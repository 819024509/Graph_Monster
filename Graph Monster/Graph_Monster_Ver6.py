...
class GraphMonster:
    """
    This version debugged the reformatter, which now performs naturally.
    This version now supports saving and loading graph data.  
    """
    ...
    def exportGraph(self):
        ...
        try:
            obj = asksaveasfile(title="Save Graph Monster Data", mode="wb")
            if obj:
                with obj as file:
    ...
    def importGraph(self):
        self.reformatState.set("Activate")
        try:
            obj = askopenfile(title="Load Graph Monster Data", mode="rb")
            if obj:
                with obj as file:
    ...
    def manipulate(self, coordBook):
        ...
        for node in self.data["Node"].values():
            ...
            fx = fy = 0
            for lineId in node.adjLines.copy():
                line = self.data["Line"][lineId]
                endNode = line.node1 if line.node2 == node else line.node2
                dfx, dfy = self.getForce(
                    self.getCenter(coordBook[node.canvasIds[0]]),
                    self.getCenter(coordBook[endNode.canvasIds[0]]),
                )
                fx += dfx
                fy += dfy
